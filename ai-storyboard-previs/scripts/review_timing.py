...
def mark(path, action, phase, kind='reviewer_wall'):
    ...
    with core.locked(path):
        ...
        if action == 'start':
            ...
            data['phases'].append(dict(phase=phase, kind=kind, started_at=utc_now(), status='running'))
        else:
            core.require(found and found['status'] == 'running', 'no running phase to end')
            found.update(ended_at=utc_now(), status='complete')
...
def main():
    ...
    ap.add_argument('command', choices=('start', 'end', 'report'))
    ...
    if a.command == 'report':
        ...
        ap.error('phase required') if not a.phase else mark(a.journal, a.command, a.phase, a.kind)
