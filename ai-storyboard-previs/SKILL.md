...
7. **最终仅图片＋MD**：render 在本地生成 240×168 小分镜图及缺图文字卡，图下标镜号。MD 固定两张表：分组表含总时长，逐镜表含镜头计划时长。建议时长逐镜标“建议”，所属组总时长标“含建议值”。单镜超过 15 秒标“需拆分或调整计划时长”，不缩短、不删镜、不冒称合格方案。缺图卡标“可推导省图”“必要漏镜”或“待检查”；逐镜展示检验、图片处理、推导依据及修改建议。**不输出视频结论表**。达到漏镜阈值，仅在两表下用一句话建议重新生成对应视频；未达到则仅逐镜列建议。格式见 [项目约定](references/project.md)。不交付视频、HTML、JSON 或内部审查记录。
...
资产一致性按 [逐镜证据门槛](references/review.md#资产外观判定逐镜结构证据) 审查：先环境干扰、再稳定结构、最后颜色光泽。帧保存 visible_facts 与 interpretation，observation 仅兼容摘要。组内一次填写 identity_scope、asset_comparisons 和问题的逐镜比较引用；服装与人物外观分别判断，不借用邻镜 FAIL，不将湿水反光单独判成换装。必要结构不可见则 uncertain 并具体补查。review_schema=3、reference_policy_version=2 进入指纹；旧结论失效，不能将旧自由文字自动升级。仍是正常三次判断与一次组 context/review。
