...
1. **依据有效**：仅 PASS 或完整补查确认 absent 的镜头进入推导判断。basis_shot_ids 是**同来源组中按镜号顺序前后夹住本镜的两个合格锚点**（各自 PASS、有当前有效选帧且 decision: anchor，相关资产图存在）。优先选择最近且足以支持剧情判断的前后锚点；邻镜缺失、未通过或不是 anchor 时向外查找。最近是选取建议，不是工具强制条件；选择较远锚点须在 reason 说明依据。不得跨来源组、不得跳乱镜序，缺失镜头和其他 ai_fill 不可充当依据，禁止循环推导或降级为单侧锚点。
...
derivable 为三值：true 表示依据有效、省去独立图后故事仍可读无歧义且无必须保护的剧情节点，可提出候选；false 表示省图会让故事断裂、有歧义或已知必须保留独立图，说明原因；null 表示证据或判断不足，说明缺什么，不能把未知写成 false。不能仅因无法唯一复原表演细节填 false/null。必要首尾及用户锁定锚点仍保留。**连续多镜缺失**时，先向缺失区间外查找合格的前后锚点，不能仅因紧邻镜也缺失就拒绝省图；各镜仍须分别满足剧情与锚点条件。若已确认必须保留独立图则填 false；若找不到某一侧合格依据且判断不足则填 null，并在 reason 写明缺哪一侧。不得把缺失镜或 ai_fill 当依据凑数，也不得用 null 掩盖已确认的必要保留。
...
输出仅两表，阈值触发时在两表下加一句建议，不生成视频结论表。最终批准 ai_fill 的已有通过镜头仍展示抽帧缩略图与原图链接；仅确认漏镜且最终批准 ai_fill 时，小分镜图栏只写“可推导生成”，不输出原图、占位图片或图片链接。两者图片处理栏均写“建议省图，未验证”。可推导漏镜检验结果仍为“视频漏镜”，已有通过镜头仍为“检验通过”。候选被最终锚点约束拒绝时继续展示图片。省图不代表原视频完整，也不删脚本位置。
