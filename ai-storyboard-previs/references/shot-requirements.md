...
同一次提取结束前，对照 shots[].script 原文和最新明确用户约束，核对 purpose、must_have/must_not_have、critical 标记及 keyframe 是否添入了原文没有的硬要求。provenance.source 引用对应镜号和关键原句；“完整连续事件”“结合上下文”本身不能证明某人必须同框或某动作必须可见。相邻镜头的人物在场、画外状态延续，不自动成为本镜入画要求。例如上一镜陈氏在门内，下一镜仅写王生牵女子入门并对陈氏说话，不能据此新增“陈氏必须同框迎接”。确属理解剧情不可缺的推断，要写明因果依据并标 assumed/unknown，不升级为 script。核对并入原有提取批次，不逐项增加代理调用。

拟判 FAIL 时再次追溯支撑该结论的原句、用户严格要求或必要剧情信息；若失败依据来自提取时自行增加的条件，先修正要求及版本，按依赖失效机制复查，不直接放行旧结论。普通构图偏差若不影响剧情、也不违反明确严格要求，不单独触发 FAIL 或返修。结构校验仅验证字段与来源标签，不能证明自然语言要求忠实于原文。

在同一次完整脚本整理中，以 purpose 表达观众必须理解的事件，以 must_have/must_not_have 记录关键人物与行动关系、道具状态、动作结果、必要空间关系及用户明确严格要求。普通景别、姿势、视角、画面左右、背景装饰与动作实施细节保留在原脚本和 keyframe 描述中，不自动转成必须精确复刻的验收条件。它们若承载关键剧情信息则仍需检查，不可一概忽略。

区分剧情节点与纯表演细节：facts/state_changes 的 critical 以及 intent.critical_result/narrative_turn 只保护首次出场、关键持物/状态变化、关键结果和转折等剧情信息。手指滑过伞柄、普通微表情等即使在脚本中单列一镜，也不因此设为 critical 或 narrative_turn；保留原描述，临时姿态放 keyframe，不杜撰为持久状态。若该细节实际承担偷换道具、关键暗号等剧情作用，须说明作用并保护。旧项目误标先依据脚本修订，再建立有效审查，不在规划时静默抹掉关键标记。

不增加逐项分级字段或逐镜 LLM 往返。原 shot_size、facts、state_changes 及其来源保持真实；宽容的是审查标准，不是改写脚本状态。已有 requirements 若把非关键描述写成硬要求，先对照原脚本与用户约束修订、提高 version，重新取得有效上下文；不能删除关键要求以制造通过。
...
- `must_have/must_not_have`：只针对目标静帧，写具体风险。视频会注明它们只在目标时刻适用，完整动作仍来自脚本与状态变化。不要写泛化负面词库。默认不把屏幕文字、对白原文、配音文本或对白口型同步写入验收条件；原脚本照原样保留，剧情含义可用于 purpose 和 event，关键画面动作及结果仍须检查；非关键动作实施细节不同但剧情等效可通过。单张关键帧不足以证明完整动作时查看同镜连续帧或视频，不通过补图代替动态证据。
...
资产的普通五官、服装颜色、材质和款式描述只作识别参考，不自动转成严格一致的验收要求。外观仅在影响角色/行动识别、身份/伪装/换装剧情线索或用户明确要求严格一致时作为必要条件；事实来源与状态继承照常保留，审查按 [剧情优先规则](review.md#资产外观判定剧情优先) 判断。

持久空间以门、窗等地标为基准；画面左右以观众为基准；左手右手以角色自身为基准。可用 `girl.world_position` 表示位置，`girl.screen_position`、`girl.body_facing_camera`、`girl.head_facing_camera`、`girl.gaze` 仅放关键帧状态。反打只改变画面投影，观察时不能把窗移墙、把持物换手或把看向某人等同于身体转向某人。是否判失败另按剧情影响判断：非关键站位/手别差异不单独失败，影响行动主体、关键持物状态、空间因果或明确严格要求时仍检查。

画外、遮挡、特写裁掉的属性继续留在持久状态，但不要求都画出来；看不见不能判定消失，也不能判定已验证。取图时优先看对应事件阶段与可见关键状态；构图只须让必要剧情信息清楚，或满足明确严格要求；身份相似或匹配成功不是图片通过。
