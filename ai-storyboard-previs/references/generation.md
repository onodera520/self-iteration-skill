...
默认视频＋脚本＋资产图流程不读取本文件，不调用 API。仅当用户另行要求生成时使用以下规则。
...
新流程 workflow: video_evidence、video_input_mode: assets。video 默认按项目 assets 顺序上传组内用到的资产图，写入 reference_field 数组；prompt_field 直接使用用户原文（多组使用 group.video_prompt 对应片段），不自动追加脚本或旧基础提示词。上传前在输入整理中核对资产顺序与提示词图片编号。无需已有逐镜分镜图。旧 video_input_mode: anchors 仍兼容按镜头顺序上传 anchor，但本次默认流程不用它。video 不接受额外 local_files，避免映射移位。模型不支持数组多图时改用支持的模型或另做明确适配，不能假装多图复现可用。
...
mute_fields 可填已确认的关闭音频参数，没有开关则留空。中间视频不交付，无需静音拼接；仍须审查视频内字幕、镜头顺序、硬切及动作结果。音频配置不代替视觉检查。
